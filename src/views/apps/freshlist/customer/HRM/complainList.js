...
class ComplainList extends React.Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      columnDefs: [
              ...
              <div className="actions cursor-pointer text-center">
                    ...
                    <Eye
                      ...
                      onClick={() =>
                        this.props.history.push({
                          pathname: `/app/ajgroup/HRM/HRMAdminForms/complainVIEWform/${params.data?._id}`,
                          state: params.data,
                        })
                      }
                    />
                ...
                {this.state.InsiderPermissions.Edit && (
                  <Edit
                    ...
                    onClick={() =>
                      this.props.history.push({
                        pathname: `/app/ajgroup/HRM/HRMAdminForms/complainEDITform/${params.data?._id}`,
                        state: params.data,
                      })
                    }
                  />
        ...
        {
          headerName: "Complaint From",
          field: "complaintFromEmployeeName",
          filter: true,
          width: 200,
          cellRendererFramework: (params) => {
            return (
              <div className="cursor-pointer text-center">
                <div>
                  <span>
                    {params.data?.complaintFromEmployeeName?.firstName}
                  </span>
                </div>
              </div>
            );
        ...
        },
        {
          ...
          cellRendererFramework: (params) => {
            return (
              <div className="cursor-pointer text-center">
                <div>
                  <span>{params.data?.complaintToEmployeeName?.firstName}</span>
                </div>
              </div>
            );
          },
  ...
  async Apicalling(id, db) {
    ...
    await _Get(COMPLIANT_HRM_VIEW, db, id)
      .then((res) => {
        this.setState({ Loading: false });
        console.log(res?.Complaint);

        if (res?.Complaint) {
          this.setState({ rowData: res?.Complaint });
        }
        this.setState({ AllcolumnDefs: this.state.columnDefs });
        this.setState({ SelectedCols: this.state.columnDefs });

        let userHeading = JSON.parse(localStorage.getItem("Complainlist"));
  ...
  async componentDidMount() {
    const InsidePermissions = CheckPermission("Complaint");
  ...
  runthisfunction(id) {
    swal("Warning", "Sure You Want to Delete it", {
      ...
      switch (value) {
        case "delete":
          _Delete(COMPLAINT_DELETE_complaint, id)
            .then((res) => {
