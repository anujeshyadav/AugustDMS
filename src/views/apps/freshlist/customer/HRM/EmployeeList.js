...
import axiosConfigThird from "../../../../../axiosConfigThird";
...
class EmployeeList extends React.Component {
                ...
                {!!params?.data?.image && (
                  <>
                    {this.state.InsiderPermissions &&
                      this.state.InsiderPermissions?.Delete && (
                        <Button
                          title="Reset User Attendance Registration"
                          className="mx-1"
                          color="danger"
                          onClick={() =>
                            this.HanldeResetRegistration(params?.data)
                          }
                          size="sm">
                          X
                        </Button>
  ...
  HanldeResetRegistration = (data) => {
    let Collectionid = `${data?.database}_${data?.branch?.branchName}`;
    let payload = {
      collectionId: Collectionid,
      faceId: data?.faceId,
    };
    axiosConfigThird
      .post(DeleteOneFace, payload)
      .then((res) => {
  ...
  async Apicalling(id, db) {
    ...
    let userHeading = JSON.parse(localStorage.getItem("EmployeeList"));
