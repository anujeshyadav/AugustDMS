...
import { Bonus_DeleteBy_Id, Bonus_List } from "../../../../../ApiEndPoint/Api";
...
class BonusList extends React.Component {
                  ...
                  <Edit
                    ...
                    onClick={() =>
                      this.props.history.push({
                        pathname: `/app/ajgroup/HRM/TCPA/BonusEdit/${params.data?._id}`,
                        state: params.data,
                      })
                    }
                  />
  ...
  async Apicalling(id, db) {
    ...
    await _Get(Bonus_List, db)
      .then((res) => {
        this.setState({ Loading: false });

        if (res?.Bonus) {
          this.setState({ rowData: res?.Bonus });
        }
        this.setState({ AllcolumnDefs: this.state.columnDefs });
        this.setState({ SelectedCols: this.state.columnDefs });

        let userHeading = JSON.parse(localStorage.getItem("Bonuslist"));
  ...
  async componentDidMount() {
    const InsidePermissions = CheckPermission("Bonus");
