...
class HolidayList extends React.Component {
          ...
          cellRendererFramework: (params) => {
            return (
              <div className="cursor-pointer text-center">
                <div>
                  <span>{params.data?.holidayName}</span>
                </div>
              </div>
            );
          },
  ...
  async Apicalling(id, db) {
    ...
    await _Get(HRM_VIEW_HOLIDAY, db)
      .then((res) => {
        this.setState({ Loading: false });

        if (res?.Holiday) {
          this.setState({ rowData: res?.Holiday });
        }
        this.setState({ AllcolumnDefs: this.state.columnDefs });
        this.setState({ SelectedCols: this.state.columnDefs });

        let userHeading = JSON.parse(localStorage.getItem("holidaylist"));
  ...
  async componentDidMount() {
    const InsidePermissions = CheckPermission("Hours & Holiday Master");
