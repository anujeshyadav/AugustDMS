...
import {
  Appraisal_DeleteBy_Id,
  Appraisal_List,
} from "../../../../../ApiEndPoint/Api";

const SelectedColums = [];

class ApprList extends React.Component {
                    ...
                    <Edit
                      ...
                      onClick={() =>
                        this.props.history.push({
                          pathname: `/app/ajgroup/HRM/TCPA/ApprformEdit/${params.data?._id}`,
                          state: params.data,
                        })
                      }
                    />
        ...
        },
        {
          ...
          cellRendererFramework: params => {
            return (
              <div className="cursor-pointer text-center">
                <div>
                  <span>{params.data?.empName?.firstName}</span>
                </div>
              </div>
            );
          },
        ...
        },
        {
          ...
          cellRendererFramework: params => {
            return (
              <div className="cursor-pointer text-center">
                <div>
                  <span>{params.data?.applyMonth}</span>
                </div>
              </div>
            );
          },
        },
        {
          ...
          cellRendererFramework: params => {
            return (
              <div className="cursor-pointer text-center">
                <div>
                  <span>{params.data?.incrementValue}</span>
                </div>
              </div>
            );
          },
  ...
  async Apicalling(id, db) {
    ...
    await _Get(Appraisal_List, db, id)
      .then(res => {
        this.setState({ Loading: false });
        if (res?.Appraisal) {
          this.setState({ rowData: res?.Appraisal });
        }
        this.setState({ AllcolumnDefs: this.state.columnDefs });
        this.setState({ SelectedCols: this.state.columnDefs });

        let userHeading = JSON.parse(localStorage.getItem("AppraisalList"));
  ...
  async componentDidMount() {
    const InsidePermissions = CheckPermission("Appraisal");
  ...
  runthisfunction(id) {
    swal("Warning", "Sure You Want to Delete it", {
      ...
      switch (value) {
        case "delete":
          _Delete(Appraisal_DeleteBy_Id, id)
            .then(res => {
