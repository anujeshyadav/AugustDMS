...
class InterviewList extends React.Component {
...

        {
          ...
          cellRendererFramework: (params) => {
            return (
              <div className="cursor-pointer text-center">
                <div>
                  <span>{params.data?.candidateName?.name}</span>
                </div>
              </div>
            );
          },
  ...
  async Apicalling(id, db) {
    ...
    await _Get(Hrm_InterviewList, db, id)
      .then((res) => {
        ...
        let userHeading = JSON.parse(localStorage.getItem("InterviewList"));
  ...
  async componentDidMount() {
    const InsidePermissions = CheckPermission("Interview");
