...
class BankStatementReport extends React.Component {
  ...
  async Apicalling(id, db) {
    ...
    await _Post(BankAccount_Report, db, id)
      .then((res) => {
        if (res?.BankAccount?.length > 0) {
          res?.BankAccount?.forEach((ele, i) => {
            let previousAmount = 0;
            if (i > 0) {
              previousAmount = res?.BankAccount[i - 1]["cashRunning"];
            }
        ...
        this.setState({
          rowData: res?.BankAccount,
          rowAllData: res?.BankAccount,
          Loading: false,
        ...
        let userHeading = JSON.parse(
          localStorage.getItem("BankStatementReportList")
        );
    ...
    //     let userHeading = JSON.parse(localStorage.getItem("BankStatementReportList"));
    //     if (userHeading?.length) {
  ...
  async componentDidMount() {
    ...
    const InsidePermissions = CheckPermission("Bank Statement");
          ...
          <Row className="ml-2 mr-2">
            ...
            <Col xl="5" lg="5" md="5">
              <Row>
                ...
                <Col lg="2" xl="2" md="2" style={{ marginTop: "30px" }}>
                  <div className="table-input">
                    <Button
                      type="submit"
                      color="primary"
                  ...
                  <div className="dropdown-container float-right">
                    ...
                    {isOpen && (
                      <div
                        ...
                        <h5
                          onClick={(e) => this.exportToExcel("BankStatement")}
                          style={{ cursor: "pointer" }}
                          className=" mx-1 myactive">
                          .XLSX
                        </h5>
