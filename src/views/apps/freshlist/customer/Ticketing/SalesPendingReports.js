...
import { view_create_order_history } from "../../../../../ApiEndPoint/Api";
...
class SalesOrderReport extends React.Component {
        ...
        },
        {
          ...
          cellRendererFramework: params => {
            return (
              <div className="text-center cursor-pointer">
                <div>
                  <span>{params.data?.order?.userId?.firstName}</span>
                </div>
              </div>
            );
          },
        ...
        },
        {
          ...
          cellRendererFramework: params => {
            return (
              <div className="text-center cursor-pointer">
                <div>
                  {params.data?.order?.createdAt &&
                    params.data?.order?.createdAt?.split("T")[0]}
                </div>
              </div>
            );
          },
  ...
  async Apicalling(id, db) {
    ...
    await _Get(view_create_order_history, id, db)
      .then(res => {
        ...
        let Alldata = res?.orderHistory?.flatMap((element, index) => {
          return element?.orderItems?.map((val, i) => {
            return { ...val, order: element };
          });
        });
        console.log(Alldata);
        let pending = Alldata?.filter(ele =>
          ele?.order.status?.toLowerCase()?.includes("pending")
        );
        this.setState({ rowData: pending });
        this.setState({ rowAllData: pending });
        this.setState({ AllcolumnDefs: this.state.columnDefs });
        this.setState({ SelectedCols: this.state.columnDefs });

        let userHeading = JSON.parse(localStorage.getItem("salesorderreports"));
  ...
  async componentDidMount() {
    ...
    const InsidePermissions = CheckPermission("SalesComplete Order Report");
  ...
  handleSubmitDate = () => {
    const filteredItems = this.state.rowAllData.filter(item => {
      const dateList = new Date(item.order?.createdAt);
