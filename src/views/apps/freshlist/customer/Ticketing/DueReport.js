...
import { Due_Report, Payment_Due_Report } from "../../../../../ApiEndPoint/Api";

const SelectedColums = [];
class DueReport extends React.Component {
      ...
      columnDefs: [
        ...
        },
        {
          ...
          cellRendererFramework: (params) => {
            return (
              <div>
                <span>{params?.data?.voucherDate}</span>
              </div>
            );
          },
        },
        ...
        },
        {
          ...
          cellRendererFramework: (params) => {
            return (
              <div>
                <span>{params?.data?.overDueDays}</span>
              </div>
            );
          },
        },
          ...
          cellRendererFramework: (params) => {
            return (
              <div>
                <span>
                  {" "}
                  <strong>{params?.data?.salesPerson}</strong>
                </span>
              </div>
            );
          },
  ...
  async Apicalling(id, db) {
    ...
    // await _Get(Due_Report, db)
    await _Post(Payment_Due_Report, db)
      .then((res) => {
        console.log(res?.PaymentDueReport);
        this.setState({ rowData: res?.PaymentDueReport });
        this.setState({ rowAllData: res?.PaymentDueReport });
        ...
        let userHeading = JSON.parse(localStorage.getItem("DueReportList"));
  ...
  async componentDidMount() {
    ...
    const InsidePermissions = CheckPermission("Due Report");
  ...
  convertCSVtoExcel = () => {
    ...
    Papa.parse(CsvData, {
      complete: (result) => {
        ...
        const excelType = "xls";
        XLSX.writeFile(wb, `dueReport.${excelType}`);
      },
    });
  };
