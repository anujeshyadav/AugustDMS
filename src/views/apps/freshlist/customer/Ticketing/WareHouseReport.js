...
import { WareHouse_Report } from "../../../../../ApiEndPoint/Api";
...
class WareHouseReport extends React.Component {
      ...
      columnDefs: [
        ...
        },
        {
          ...
          cellRendererFramework: (params) => {
            return (
              <div className="actions cursor-pointer text-center">
                <span>{params?.data?.closingStock}</span>
              </div>
            );
          },
        },

        {
          ...
          cellRendererFramework: (params) => {
            return (
              <div className="actions cursor-pointer text-center">
                <span>{params?.data?.damageItem?.currentStock}</span>
              </div>
            );
          },
        },
  ...
  async Apicalling(id, db) {
    ...
    await _Post(WareHouse_Report, db, id)
      .then((res) => {
        ...
        const mappedData = value
          .map((item) => {
            ...
            return entries
              ...
              .map(([key, value]) => ({
                ...value,
                uniqueKey: key, // Preserving the unique key if needed
                groupId: item._id, // This could help group items by their original container
              }));
          })
          .flat();
        ...
        if (mappedData?.length > 0) {
          ...
          this.setState({ rowAllData: mappedData });
        }
        this.setState({ AllcolumnDefs: this.state.columnDefs });
        this.setState({ SelectedCols: this.state.columnDefs });

        let userHeading = JSON.parse(
          localStorage.getItem("warehousereportlist")
        );
  ...
  async componentDidMount() {
    ...
    const InsidePermissions = CheckPermission("WareHouse Reports");
