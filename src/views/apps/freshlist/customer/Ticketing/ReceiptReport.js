...
class ReceiptReport extends React.Component {
    ...
    await _Get(View_Receipt, db)
      .then((res) => {
        ...
        if (Received?.length > 0) {
          Received?.forEach((ele, i) => {
            let previousAmount = 0;
            if (i > 0) {
              previousAmount = Received[i - 1]["cashRunning"];
            }
        ...
        if (Received?.length) {
          this.setState({ rowData: Received, rowAllData: Received });
        }
        this.setState({ AllcolumnDefs: this.state.columnDefs });
        this.setState({ SelectedCols: this.state.columnDefs });

        let userHeading = JSON.parse(localStorage.getItem("ReceiptReportlist"));
  ...
  async componentDidMount() {
    ...
    const InsidePermissions = CheckPermission("receipt Report");
  ...
  generatePDF(parsedData) {
    ...
    doc.text(`Receipt Report-${date}`, 14, 51);
    doc.autoTable({
  ...
  convertCSVtoExcel = () => {
    ...
    Papa.parse(CsvData, {
      complete: (result) => {
        ...
        const excelType = "xls";
        XLSX.writeFile(wb, `ReceiptReport.${excelType}`);
      },
    });
  };
