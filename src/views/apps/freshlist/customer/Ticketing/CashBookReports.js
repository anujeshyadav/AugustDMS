...
class CashBookReports extends React.Component {
    ...
    this.state = {
      ...
      columnDefs: [
        ...
        },
        {
          ...
          filter: true,
          valueGetter: (params) => {
            if (params?.data?.type == "receipt") return params?.data?.amount;
              ...
              <div className="d-flex align-items-center cursor-pointer">
                <div>
                  <span color="primary">
                    {" "}
                    <strong>
                      {params?.data?.cashRunning && params?.data?.cashRunning}
                    </strong>
                  </span>
                </div>
              </div>
  ...
  async Apicalling(id, db) {
    ...
    await _Post(Cashbook_Report, db, id)
      .then((res) => {
        this.setState({ Loading: false });
        if (res?.CashBook?.length > 0) {
          res?.CashBook?.forEach((ele, i) => {
            let previousAmount = 0;
            if (i > 0) {
              previousAmount = res?.CashBook[i - 1]["cashRunning"];
            }
            let currentAmount = ele?.amount;
            // let type = ele.type == "payment" ? "-" : "+";
            if (ele.type == "payment") {
        ...
        if (res?.CashBook?.length) {
          this.setState({ Loading: false });
          this.setState({ rowData: res?.CashBook, rowAllData: res?.CashBook });
        }
        this.setState({ AllcolumnDefs: this.state.columnDefs });
        this.setState({ SelectedCols: this.state.columnDefs });

        let userHeading = JSON.parse(
          localStorage.getItem("cashbookreportlist")
        );
    ...
    //     let userHeading = JSON.parse(localStorage.getItem("cashbookreportlist"));
    //     if (userHeading?.length) {
    //       this.setState({ columnDefs: userHeading });
    //       // this.gridApi.setColumnDefs(userHeading);
    //       this.setState({ SelectedcolumnDefs: userHeading });
  ...
  async componentDidMount() {
    ...
    const InsidePermissions = CheckPermission("cashbook Report");
  ...
  downloadExcelFile(blob) {
    ...
    a.href = url;
    a.download = "CashBookReport.xlsx";
  ...
  convertCSVtoExcel = () => {
    ...
    Papa.parse(CsvData, {
      complete: (result) => {
        ...
        const excelType = "xls";
        XLSX.writeFile(wb, `CashbookReport.${excelType}`);
      },
    });
  };
                  ...
                  <div className="dropdown-container float-right">
                    ...
                    {isOpen && (
                      <div
                        ...
                        <h5
                          onClick={(e) => this.exportToExcel("CashbookReport")}
                          style={{ cursor: "pointer" }}
                          className=" mx-1 myactive">
                          .XLSX
                        </h5>
