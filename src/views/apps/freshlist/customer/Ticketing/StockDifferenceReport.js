...
import { WarehouseDifference_Report } from "../../../../../ApiEndPoint/Api";

const SelectedColums = [];

class StockDifferenceReport extends React.Component {
    ...
    this.state = {
      ...
      columnDefs: [
...

        {
          ...
          cellRendererFramework: params => {
            return (
              <>
                <div className="actions cursor-pointer text-center">
                  <span>{params?.data?.openingStock}</span>
                </div>
              </>
            );
          },
        },
        {
          ...
          cellRendererFramework: params => {
            return (
              <>
                <div className="actions cursor-pointer text-center">
                  <span>{params?.data?.warehouseStock}</span>
                </div>
              </>
            );
          },
          ...
          cellRendererFramework: params => {
            return (
              <>
                <div className="actions cursor-pointer text-center">
                  <span>
                    {params?.data?.damageItem && params?.data?.damageItem}
                  </span>
                </div>
              </>
            );
        ...
        },
        {
          ...
          cellRendererFramework: params => {
            let dif =
              params.data?.openingStock -
              (params.data?.damageItem +
                (params.data?.openingStock - params.data?.warehouseStock));
  ...
  async Apicalling(id, db) {
    ...
    await _Post(WarehouseDifference_Report, db, id)
      .then(res => {
        this.setState({ Loading: false });
        let value = res?.Warehouse[0];
        const stockArray = Object.entries(value).map(([key, value]) => {
          return {
            id: key,
            ...value,
          };
        ...
        if (stockArray?.length > 0) {
          this.setState({ rowData: stockArray });
          this.setState({ rowAllData: stockArray });
  ...
  async componentDidMount() {
    ...
    const InsidePermissions = CheckPermission("STOCK DIFFERENCE REPORT");
...
export default StockDifferenceReport;
