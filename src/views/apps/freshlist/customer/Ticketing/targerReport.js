...
import {
  Create_Account_List,
  SalesPerson_targetReport,
} from "../../../../../ApiEndPoint/Api";
import SalesPersonTarget from "./SalesPersonTarget";
...
class targerReport extends React.Component {
  ...
  handleChangeView = async (data) => {
    this.setState({ Loading: true });
    let url = `${SalesPerson_targetReport + data?._id}/`;
    await _Post(url, data?.database)
      .then((res) => {
        this.setState({ Loading: false });
        this.setState({
          ViewOneData: { data: data, achievements: res?.achievements },
        });
        this.togglemodal();
      })
      .catch((err) => {
  ...
  exportToPDF = async () => {
    ...
    await exportDataToPDF(csvData, "TargetReportList");
  };
  processCell = (params) => {
    return params.value;
  };
  ...
  exportToExcel = async (e) => {
    ...
    await exportDataToExcel(CsvData, "TargetReportList");
  };
  ...
  convertCSVtoExcel = async () => {
    ...
    await convertDataCSVtoExcel(CsvData, "TargetReportList");
  };
  ...
  convertCsvToXml = async () => {
    ...
    await convertDataCsvToXml(CsvData, "TargetReportList");
  };
      ...
      <>
                  ...
                  <Col>
                    ...
                    >
                      <input
                        ...
                        value="SalesPerson"
                      />
                      <span style={{ marginRight: "20px" }}>SalesPerson</span>
                      ...
                      <input
                        ...
                        value="Customer"
                      />
                      <span style={{ marginRight: "3px" }}>Customer</span>
                    </div>
                  </Col>
                </Row> */}
              </Col>
        ...
        <Modal
          ...
          <ModalHeader toggle={this.togglemodal}>
            {this.state.ViewOneData?.firstName} Target
          </ModalHeader>
          <ModalBody className="">
            <SalesPersonTarget target={this.state.ViewOneData} />
          </ModalBody>
        </Modal>
      </>
    );
