...
import { Auto_Billing_Lock, Tax_Report } from "../../../../../ApiEndPoint/Api";
...
class TaxReport extends React.Component {
          ...
          cellRendererFramework: (params) => {
            return (
              <div className="text-center cursor-pointer">
                <span>
                  <strong>
                    {params?.data?.totalTaxInput &&
                      params?.data?.totalTaxInput?.toFixed(2)}
                  </strong>
                </span>
              </div>
            );
          ...
          cellRendererFramework: (params) => {
            return (
              <div className="text-center cursor-pointer">
                <span>
                  <strong>
                    {params?.data?.totalTaxOut &&
                      params?.data?.totalTaxOut?.toFixed(2)}
                  </strong>
                </span>
              </div>
            );
  ...
  async Apicalling(id, db) {
    ...
    await _Post(Tax_Report, db, id)
      .then((res) => {
        ...
        let userHeading = JSON.parse(localStorage.getItem("TaxReportList"));
  ...
  async componentDidMount() {
    ...
    const InsidePermissions = CheckPermission("TAX REPORT");
  ...
  convertCSVtoExcel = () => {
    ...
    Papa.parse(CsvData, {
      complete: (result) => {
        ...
        const excelType = "xls";
        XLSX.writeFile(wb, `Tax.${excelType}`);
      },
    });
  };
      ...
      <>
        <Card>
          <Row className="ml-2  mr-2">
            ...
            <Col style={{ marginTop: "30px" }} lg="1" xl="1" md="1">
