...
const UpdatePurchaseOrder = (args) => {
    ...
    (async () => {
      ...
      await _Get(Purchase_OrderViewOne, Params?.id)
        .then((res) => {
          ...
          values.transporterId = values.transporter?._id?._id
            ? values.transporter?._id?._id
            : "";
          setData(values);

          setExtraCharges(values);
  ...
  const submitHandler = async (e) => {
    ...
    const payload = {
      coolieAndCartage: Charges ? Charges : 0,
      invoiceId: extraCharges?.invoiceId,
      BuiltyNumber: extraCharges?.BuiltyNumber,
      DispatchDate: extraCharges?.DispatchDate,
      NoOfPackage: extraCharges?.NoOfPackage,
      transporter: extraCharges?.transporter,
      vehicleNo: extraCharges?.vehicleNo,
    };
    await _Put(Purchase_AddInforEdit_Order, Params.id, payload)
      .then((res) => {
        History.goBack();
