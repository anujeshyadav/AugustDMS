...
import { Auto_Billing_Lock } from "../../../../ApiEndPoint/Api";
import {
...
class OrderSearch extends React.Component {
  ...
  async Apicalling(id, db) {
    ...
    await _Get(Auto_Billing_Lock, db)
      .then((res) => {
        this.setState({ Loading: false });
        console.log(res?.customer);
        this.setState({ Loading: false });
        this.setState({ rowData: res?.customer?.reverse() });

        this.setState({ AllcolumnDefs: this.state.columnDefs });
        this.setState({ SelectedCols: this.state.columnDefs });

        let userHeading = JSON.parse(localStorage.getItem("BillingLock"));
  ...
  async componentDidMount() {
    ...
    const InsidePermissions = CheckPermission("Auto Billing Lock");
  ...
  exportToPDF = async () => {
    ...
    await exportDataToPDF(csvData, "BliingLockList");
  };
  processCell = (params) => {
    return params.value;
  };
  ...
  exportToExcel = async (e) => {
    ...
    await exportDataToExcel(CsvData, "BliingLockList");
  };
  ...
  convertCSVtoExcel = async () => {
    ...
    await convertDataCSVtoExcel(CsvData, "BliingLockList");
  };
  ...
  convertCsvToXml = async () => {
    ...
    await convertDataCsvToXml(CsvData, "BliingLockList");
  };
